defmodule Advent19.IntcodeV7 do
  @moduledoc """
  This is the resulting IntCode Computer after Day 7,
  which needs to run multiple programs at once
  """
  ...
  defp do_compute([{3, _, _, _}, update_pos | _tail], program, [input | remaining_input], output, instruction_pointer) do
    program
    |> List.update_at(update_pos, fn _ -> input end)
    |> compute(remaining_input, output, instruction_pointer + 2)
  end

  # Opcode 4: Write to output
  defp do_compute([{4, p1m, _, _}, p1 | _tail], program, _input, output, instruction_pointer) do
    output_value = if(p1m == :imm, do: p1, else: Enum.at(program, p1))
    output = [output_value | output]

    # {:output, 42, instruction_pointer to resume at and memory dump}
    {:output, output, {instruction_pointer + 2, program}}
  end
  ...
  defp do_compute([{99, _, _, _} | _], _program, _input, output, _instruction_count),
    do: {:halt, output |> Enum.reverse()}
